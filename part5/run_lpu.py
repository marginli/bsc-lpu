...
def step09(brain: Brain, force: bool = False) -> dict:
    ...
    if partial == 0:
        print()
        print("┌" + "─" * 70)
        print("│ 照補充材料的滾雪球讀法，這一步沒有可用的操作區間。")
        print(f"│ {total} 種參數組合，每一種的結果不是 1（只有種子）就是 {len(ids)}（整池）。")
        print("│ 沒有任何一種設定收到「一部分」。")
        print("│")
        print("│ 這不是參數沒調好，是這個讀法本身的性質：每收一顆就把靶放大一次，")
        print("│ 所以它只有兩個終點——收不到任何一顆，或者一路收到底。")
        print("│ 論文對這一步還有另一種寫法（圖 S5D(e)：靶是分群本身，固定不動），")
        print("│ 那一種的結果會隨門檻連續變化。這支程式實作的是補充材料那一種。")
        print("│")
        print("│ 主程式在這裡停下來，不往下產出看起來完整、其實沒有意義的結果。")
        print("│ 要繼續，請加上 --force-recruit：接下來的一切都建立在")
        print(f"│ 「撐大 {cfg.grow} 格」這一種設定上，那是跑得動的設定，不是驗證過的。")
        print("└" + "─" * 70)
        if not force:
